...
An existing config named `netpulse.conf` is still untrusted until marked. When an owner has independently verified it is the dedicated instance, the only permitted adoption is a root-only, explicit-confirmation, backup-first marker addition that validates the dedicated TCP/TUN profile and makes no service or listener change. After adoption, normal repair can start the marked unit. If client certificates were issued by a different CA, sync the installed dedicated service CA into NetPulse and reprovision affected routers rather than replacing the live service PKI.